...
<Meta title="Guides/Upgrade Guides/v3.0" />
<Unstyled>
...
## Canvas Token v2.0 -> v3.0

> **Important:** v3.0 tokens use `oklch` instead of `RGBA` which will result in colors being perceived differently. The new color palette is always a **breaking change**. To reduce the impact, we strongly suggest using `system` or `brand` tokens instead of `base` tokens.

If you're migrating from v2.0 to v3.0, the biggest changes will be:
- Colors now use `oklch` instead of `RGBA`
- New base palette: `blue`, `green`, `red`, `amber`, `purple`, `orange`, `neutral`, `azure`, `magenta`, `teal`, `indigo`, `coral` and `slate`.
- Old "fruit" based colors are deprecated and map to the new colors. 
